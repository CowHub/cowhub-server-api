...
S3_PATH="s3://$DEPLOYMENT_BUCKET/$BUCKET_DIRECTORY/$SOURCE_BUNDLE"
...
# Bundle
cd scripts && cp -r ../.ebextensions ./
cd scripts && zip -r $SOURCE_BUNDLE.zip .ebextensions Dockerrun.aws.json

# Uploading json file to $S3_PATH
aws s3 cp ./scripts/$SOURCE_BUNDLE.zip $S3_PATH
